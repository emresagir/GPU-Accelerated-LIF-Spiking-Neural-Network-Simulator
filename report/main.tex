...
\subsection{CPU Model}
  The CPU implementation is being done with 
  the given neuron and synapse model definitions in previous chapters.
  It is written in C. 
  The main time loop given in the figure~\ref{fig:C_impl}. \\
  
  To check the correctness of the C model, 
  Pytorch version is also implemented with the same model definitons.
  The comparison test done. 
  Firstly, the poisson (input) spikes are saved while running
  the pytorch version. Then those spikes are imported to the C version. 
  This way the model expected to be run exactly the same. 
  Meanwhile membrane potentials (u[t]) and spikes (s[t]) 
  are being saved in both versions.
  At the end those saved values are being compared with a script,
  to check the correctness of the C implementation. 
...
\end{lstlisting}
\caption{Time loop of the C implementation}
\label{fig:C_impl}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{comp_C_vs_torch.png}
  \caption{Comparison output of C and pytorch versions}
