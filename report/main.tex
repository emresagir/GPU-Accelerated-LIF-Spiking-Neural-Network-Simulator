...
In this naive version, there are couple of problems that are apparent. There is no kernel fusion; two seperate kernel introduces an overhead.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{Python_C_GPU_membrane_comparison.png}
  \caption{Membrane Potential comperison of Torch, CPU and GPU versions}
  \label{fig:membrane_comp_Torch_CPU_GPU}
\end{figure}

The next step would be to show the absolute error of the GPU, how much it diverges from the reference. 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{Python_GPU_Absolute_error.png}
